...
def _validate_contribution(kind: object, points: object, handle: object, title: object, url: object, source_ref: object, impact_multiplier: object) -> tuple[str, int, str, str, str | None, str, int]:
...
def _insert_contribution(connection: sqlite3.Connection, *, ts: str, kind: str, points: int, handle: str, title: str, url: str | None, source_ref: str, impact_multiplier: int) -> bool:
    cursor = connection.execute(
        """
        INSERT OR IGNORE INTO contributions
          (ts, kind, points, handle, url, title, source_ref, impact_multiplier)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (ts, kind, points, handle, url, title, source_ref, impact_multiplier),
    )
...
def add_contribution(kind: str, points: int, handle: str, title: str, *, url: str | None = None, source_ref: str | None = None, ts: str | datetime | None = None, impact_multiplier: int = 1, db_path: Path | None = None, ledger_path: Path | None = None) -> dict[str, Any]:
    if source_ref is None:
        source_ref = f"manual:{uuid.uuid4()}"
    validated = _validate_contribution(kind, points, handle, title, url, source_ref, impact_multiplier)
    ...
    try:
        ...
        return {"id": row["id"], "inserted": inserted, "ts": row["ts"], "kind": row["kind"], "points": row["points"], "handle": row["handle"], "title": row["title"], "url": row["url"], "source_ref": row["source_ref"], "impact_multiplier": row["impact_multiplier"]}
    finally:
        connection.close()
...
def _ranked(rows: list[sqlite3.Row]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        handle = row["handle"]
        entry = grouped.setdefault(handle, {"handle": handle, "points": 0, "first_ts": row["ts"], "first_id": row["id"], "url": None, "breakdown": {}, "contributions": []})
...
def board_snapshot(db_path: Path | None = None, *, now: datetime | None = None, ledger_path: Path | None = None) -> dict[str, list[dict[str, Any]]]:
    ...
    try:
        ...
        ticker = [_contribution_item(row) for row in sorted(rows, key=lambda row: (row["ts"], row["id"]), reverse=True)[:12]]
        return {"today": _ranked(today_rows), "alltime": _ranked(rows), "ticker": ticker}
    finally:
        connection.close()
