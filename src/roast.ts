...
export function renderRoast(root: HTMLElement): () => void {
  ...
  root.innerHTML = `<main class="roast-page roast-skin-${intensity}"><div class="site-shell">${siteNav('light')}<header class="roast-hero"><p class="eyebrow">THE ROAST · GORIA IS WATCHING</p><h1>Drop your URL.<br>Get the receipt.</h1><p class="business-intro">A roast of what your website actually says, shows, and loads. Every burn comes with the exact observation behind it.</p><form class="roast-form" data-roast-form><label>Your domain<input name="domain" type="text" placeholder="yourdomain.com" autocomplete="url" required maxlength="253"></label><fieldset><legend>How much truth can it take?</legend><div class="intensity-row"><label class="intensity-option"><input type="radio" name="intensity" value="gentle">Gentle<span>Sharp, never cruel.</span></label><label class="intensity-option"><input type="radio" name="intensity" value="honest" checked>Honest<span>The useful version.</span></label><label class="intensity-option intensity-scorched"><input type="radio" name="intensity" value="scorched">Scorched Earth<span>Bring the fire.</span></label></div></fieldset><button class="button button-primary" type="submit">Roast my site →</button></form><p class="roast-note">No screenshot. No invented claims. Just the page and its receipts.</p></header><div data-roast-output></div><footer class="site-footer"><strong>Human-directed, AI-executed.</strong><span>Want to improve the organism?</span><a href="/board">Board</a><a href="/rules">Rules</a><button class="footer-action" data-action="improve">Improve this</button><a href="/">Play the Living Pitch →</a></footer></div></main>`
