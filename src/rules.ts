...
const KNOWN_CATEGORIES = ['dev', 'copy', 'seo', 'design', 'business', 'qa']
...
export function renderRules(root: HTMLElement): void {
  ...
  void fetch(`${API_BASE}/board`, { headers: { Accept: 'application/json' }, signal: AbortSignal.timeout(10000) })
    .then(async (response) => {
      ...
      if (target && categories.length > 0) target.innerHTML = renderCategoryRules(categories)
      capture('rules_view', { categories: categories.length })
    })
    .catch((error: unknown) => {
      // The static list above stays in place; the live list is only an upgrade.
