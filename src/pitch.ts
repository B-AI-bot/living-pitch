...
function sceneIndex(scene: SceneId): number {
  return ({ basecamp: 0, pipeline: 1, 'follow-through': 2, summit: 5 })[scene]
}
...
function renderScene(state: PitchState): string {
  ...
  const questions = state.scene === 'pipeline' || state.scene === 'follow-through' ? sceneQuestions[state.scene].map((id) => scanQuestion(id, state)).join('') : ''
